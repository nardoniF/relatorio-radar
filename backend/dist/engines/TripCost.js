...
export function computeTripCost(input) {
    ...
    const withFine = input.violationEstimates.filter((v) => v.estimatedFineBrl != null && v.estimatedFineBrl > 0);
    const estimatedFinesBrl = withFine.reduce((s, v) => s + (v.estimatedFineBrl ?? 0), 0);
    return {
        distanceKm,
        fuelLiters,
        ...
        disclaimer: 'Totais incluem estimativa de possíveis infrações — não constituem autuação oficial.',
    };
}
function round2(n) {
    return Math.round(n * 100) / 100;
}
//# sourceMappingURL=TripCost.js.map