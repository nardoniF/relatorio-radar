...
export class RadarDetection {
    ...
    update(sample) {
        let bestAlert = null;
        let newPassage = null;
        const ignoredOpposite = [];
        for (const radar of this.radars) {
            if (!radar.active)
                continue;
            if (this.passedIds.has(radar.id))
                continue;
            const opposite = isOppositeDirection(sample.headingDeg, radar.directionDeg, radar.directionToleranceDeg);
            if (opposite) {
                ignoredOpposite.push(radar.id);
                continue;
            }
            const sameDir = headingMatches(sample.headingDeg, radar.directionDeg, radar.directionToleranceDeg);
            if (!sameDir) {
                ignoredOpposite.push(radar.id);
                continue;
            }
            ...
            if (d <= radar.alertRadiusM) {
                const level = d <= radar.passRadiusM * 1.4
                    ? 'imminent'
...
//# sourceMappingURL=RadarDetection.js.map