...
export function headingMatches(
  vehicleHeading: number | null | undefined,
  radarDirectionDeg: number | null | undefined,
  toleranceDeg = 60,
): boolean {
  if (radarDirectionDeg == null) return true
  if (vehicleHeading == null || !Number.isFinite(vehicleHeading)) return true
...
export function isOppositeDirection(
  ...
  if (radarDirectionDeg == null) return false
  if (vehicleHeading == null || !Number.isFinite(vehicleHeading)) return false
